...
class StopperState extends State<Stopper> with SingleTickerProviderStateMixin {
  ...
  set stop(nextStop) {
    _targetStop = max(0, min(_stops.length - 1, nextStop as int));
    _animate(height as double, nextStop as double);
  }
  ...
  void close() {
    if (!_closing && canClose) {
      _closingHeight = height as double;
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
        ...
        builder: (context, child) {
          return SizedBox(
            height: min(_stops[_stops.length - 1], max(0, height as double)),
            child: child,
          );
        });
